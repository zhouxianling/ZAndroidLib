...
public class LCheckBox extends View implements Checkable {
    ...
    public LCheckBox(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public LCheckBox(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.LCheckBox);
        int tickColor = ta.getColor(R.styleable.LCheckBox_color_tick, COLOR_TICK);
        mAnimDuration = ta.getInt(R.styleable.LCheckBox_duration, DEF_ANIM_DURATION);
        mFloorColor = ta.getColor(R.styleable.LCheckBox_color_unchecked_stroke, COLOR_FLOOR_UNCHECKED);
        mCheckedColor = ta.getColor(R.styleable.LCheckBox_color_checked, COLOR_CHECKED);
        mUnCheckedColor = ta.getColor(R.styleable.LCheckBox_color_unchecked, COLOR_UNCHECKED);
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.LCheckBox_stroke_width, LDensityTool.dp2px(getContext(), 0));
        ta.recycle();
    ...
    public void setChecked(boolean checked) {
        ...
        invalidate();
        if (mListener != null) {
            mListener.onCheckedChanged(LCheckBox.this, mChecked);
        }
    }
    ...
    private int measureSize(int measureSpec) {
        int defSize = LDensityTool.dp2px(getContext(), DEF_DRAW_SIZE);
    ...
    public interface OnCheckedChangeListener {
        void onCheckedChanged(LCheckBox checkBox, boolean isChecked);
    }
}
